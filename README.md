...
princp=[]
try:
    prox= requests.get('https://api.proxyscrape.com/?request=displayproxies&protocol=socks5&timeout={max_proxy}&country=all&ssl=all&anonymity=all).text
...
def banner():
    ...
    {m}•{k}•{h}•{sir} RECODE : ENGGAR {x}{m}•{k}•{h}•{x}''')
#--------------------[ BAGIAN-MASUK ]--------------#
def login():
	try:
